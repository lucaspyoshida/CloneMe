...
class LegendaInput(BaseModel):
    imagem_descricao: str = Field(description="A descrição textual e detalhada da imagem para a qual a legenda será criada.")


criador_de_legendas_para_imagens = Agent(
    name="criador_de_legendas_para_imagens",
...
1.  **Análise Total do Perfil:** Estude CADA CAMPO do JSON em `<perfil>`. Entenda profundamente as regras de linguagem, tom, vocabulário, estrutura, pontuação, emojis e hashtags.
...
* **Fidelidade Total ao Perfil:** Siga 100% das regras especificadas no `<perfil>`. Se um campo estiver ausente, vazio ou com contagem zero (ex: `emoji_profile.avg_count` = 0), NÃO use esse elemento. Não invente nada que não esteja explicitamente definido.
